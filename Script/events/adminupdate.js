        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`»» NOTICE «« Update user ${logMessageData.TARGET_ID} NOW YOU CAN ALSO CONTOL THIS GRUOP OKH 😚:𝐙𝐈𝐒𝐀𝐍 𝐀𝐇𝐌𝐄𝐃:🫵🏾`, threadID, async (error, info) => {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`»» NOTICE «« Update user ${logMessageData.TARGET_ID} You Are Not Perfect For The Post Admin \n That Why You Removed From Admin :𝐙𝐈𝐒𝐀𝐍 𝐀𝐇𝐌𝐄𝐃:|`, threadID, async (error, info) => {
                        if (global.configModule[this.config.name].autoUnsend) {
                            await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                            return api.unsendMessage(info.messageID);
                        } else return;
            ...
            case "log:thread-icon": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`» [ Why Are You Send Like Mad 🤬 ] y.replace("emoji", "icon")}\n» Original icon: ${preIcon[threadID] || "unknown"}`, threadID, async (error, info) => {
